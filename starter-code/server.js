...
// We are declaring a constant variable with 'const' named client and using the assingment operator, we are creating a new postgres client with our connString passed as our argument. Essentially, we are telling the server what postgres user we are connecting as, so that our server is allowed to talk with postgres.
...
app.get('/articles', function(request, response) {
  ...
  // The query to the DB is being invoked which tells the DB to create a table if none exists called 'authors', and setting the columns by name and what type of value it holds (i.e. author_id is incremented by 1 and is a primary key to match up with other related tables; author and authorUrl are text that cannot exceed 255 characters). This code matches up with #3.
  client.query(`
  ...
  client.query(`
    ...
    // The primary key means that the article_id column will have unique information so we can use it to identify a specific row. Because it is in a query, it corresponds to #3.
    // +++++++++++++++++++++
    // What does VARCHAR mean?
    // The expression will be limited to a specified number of characters. Any inputs greater than the number will be truncated. Again, since it's in a query, matches to #3.
    // +++++++++++++++++++++
...
// A 'get' route is a select or read action, where 'post' is a insert or create action. app.post is handling a request from the view, so it matches to #2.
app.post('/articles', function(request, response) {
  ...
  function queryTwo() {
    client.query(
      ...
      // $1 is identifing what input from the request to use in the SQL query. In this case it is request.body.author. This is in the query, which matches to #3, but the information is from the request, which matches #2.
  ...
  function queryThree(author_id) {
      // DONE: What number in the full-stack diagram best matches what is happening in line 100? The client query is a SQL query, from the server to the DB, so it matches #3.
    client.query(
      `INSERT INTO
      ...
      function(err) {
        if (err) console.error(err);
        // DONE: What number in the full-stack diagram best matches what is happening in line 114? A response is being sent from the server to the view layer, so it matches #5.
...
app.put('/articles/:id', function(request, response) {
  ...
  function queryTwo(author_id) {
    client.query(
      // DONE: In a sentence or two, describe how a SQL 'UPDATE' is different from an 'INSERT', and identify which REST verbs and which CRUD components align with them. 'UPDATE' (Update from CRUD) is changing a record that already exists, where 'INSERT' (Create from CRUD) is creating a new record that didn't previously exist. Matches #3.
      `UPDATE authors
  ...
  // DONE: What number in the full-stack diagram best matches what is happening in line 163? It is handling a request from the view layer, so it matches #2.
app.delete('/articles/:id', function(request, response) {
    // DONE: What number in the full-stack diagram best matches what is happening in lines 165? It is a query to the DB (model layer), so it matches #3.
  client.query(
    `DELETE FROM articles WHERE article_id=$1;`,
    // DONE: What does the value in 'request.params.id' come from? If unsure, look in the Express docs.
    // It comes from the request. 'params' is an object with a property 'id' that it gets from the route. I actually have no idea how to utilize this though. It takes in the parameters from the request (matches #2), but is using it for the DB query (matches #3).
    [request.params.id]
  );
  // DONE: What number in the full-stack diagram best matches what is happening in line 172? The server (controller layer) is sending a response to the view layer, so it matches #5.
